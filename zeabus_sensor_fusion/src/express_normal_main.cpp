...
int main( int argc , char** argv){
	...
	while( nh.ok()){
        ...
        current_velocity_x = auv_state.twist.twist.linear.x;
        current_velocity_y = auv_state.twist.twist.linear.y;
		if( get_dvl && get_imu && get_pressure){
			...
			if( absolute_check(adding_y)) auv_state.pose.pose.position.y += adding_y;
                ...
                ROS_INFO("dvl_data\t: %.4lf\t%.4lf", current_velocity_x , current_velocity_y );
