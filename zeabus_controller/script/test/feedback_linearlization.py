...
# ( m , i_xx , i_yy , i_zz ) argument of coriolis_rb
		self.c_rb = coriolis_rb(	0	,1		,2		,3		)

# ( a_1 , a_2 , a_3 , b_1 , b_2 , b_3 ) argument for input constant
		self.c_a = coriolis_a( 	[1 , 2 , 3]		, [4 , 5 , 6]		, [7 , 8 , 9]
								[9 , 8 , 7]		, [6 , 5 , 4]		, [3 , 2 , 1]
							 )

# ( x , y , z , k , m , n)
		self.d_i = hydrodynamic_i(	1,	2,	3,	4,	5,	6)
# ( x , y , z , k , m , n)
		self.d_q = hydrodynamic_q( 	1,	2,	3,	4,	5,	6)

# (w , b , x_cm , x_cb , y_cm , y_cb , z_cm , z_cb):
		self.g_n = gravitational_buoyancy_force( 10 , -5 , 1 , 2 , 3 , 4 , 5 , 6)
	...
	def listen_state( self , message ):
		...
		self.velocity[4] = message.twist.twist.linear.x
		self.velocity[5] = message.twist.twist.linear.y
		self.velocity[6] = message.twist.twist.linear.z
	...
	def listen_cmd_vel( self , message):		
		...
		self.output = self.thruster  
					  	-	( self.c_rb.input_value( self.velocity[0]
												,self.velocity[1]
												,self.velocity[2])
						...
						- self.g_n.input_value( self.position[3] , self.position[4] 
												, self.position[5])

		self.publish_thruster( convert_output() )
