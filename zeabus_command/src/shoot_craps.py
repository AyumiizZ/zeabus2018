...
'''
    For limit time in this mission    
'''
...
class Timer(object):
    ...
    def isTimeout(self):
        ...
        if t >= self.timeout:
            ...
            print_result('Remaining timeout :' + '%.2f' %
                         (self.timeout - t) + 'second')
            return False

    '''
        noOfIncreasing is the variable that limit number of time increasing
    '''
...
class EstimatePosition():
    ...
    def __init__(self, x, y, z, yaw):
        '''
            ...
            - Task A (Maker) to Task B (This task)
            - Relative distance between task A and B
            - Previous state is absolute position after finish task A
            - Next state is absolute position of task B

        '''
        self.aicontrol = AIControl()
        self.relativeDistance = {'x': x, 'y': y, 'z': z, 'yaw': yaw}
        self.previousState = {'x': 0, 'y': 0, 'z': z, 'yaw': 0}
        self.nextState = {'x': 0, 'y': 0, 'z': z, 'yaw': 0}
    ...
    def moveToEstimatePos(self):
        self.nextState['x'] = self.previousState['x'] + \
            self.relativeDistance['x']
        self.nextState['y'] = self.previousState['y'] + \
            self.relativeDistance['y']
        self.nextState['z'] = self.previousState['z'] + \
            self.relativeDistance['z']
        self.nextState['yaw'] = self.previousState['yaw'] + \
            self.relativeDistance['yaw']
        auv = self.aicontrol
        auv.fixXY(self.checkPoint['x'], self.checkPoint['y'])
        auv.depthAbs(self.checkPoint['z'])
...
class ShootCraps(object):
                    ...
                    if not checkCenter[1]:
                        ...
                        if cy < 0:
                            ...
                            for i in range(int(cy * 7.5)):
                                self.move('down', 1)
                        auv.depthRelative(0)
                    if checkCenter[0] == True and checkCenter[1] == True:
                        if self.isHit(point):
    ...
    '''
        1. Find checkpoint
        2. hit 5
        3. if hit 5 fail -> hit 6 -> hit 6 again -> "Next Mission" 
        4. else hit 6 -> hit 5 -> "Next Mission"
    '''
    
    def run(self):
        print_result('MAKER FINISHED')
        '''
        EP = EstimatePosition()
        EP.setPreviousState()            
        '''
        
        print_result('SHOOT CRAPS RUNNING')
        nextState = self.findCheckPoint()
        '''
        if not nextState:
            EP.moveToEstimatePos()
        '''
        nextState = self.hitDice(5)
        self.hitDiceFail(nextState)
        if nextState:
            nextState = self.hitDice(6)
            self.hitDiceFail(nextState)
            nextState = self.hitDice(6)
        else:
            nextState = self.hitDice(6)
