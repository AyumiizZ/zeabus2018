...
class AIControl:
    ...
    def driveX(self, x, err=0.2):
        print 'doing drive x'
        self.stop()
        count = 0
        reset = 0
        while not rospy.is_shutdown() and count < 10:
            if self.srv_ok_position(String('xy'), err).ok and self.srv_ok_position(String('yaw'), err).ok:
                count += 1
                rospy.sleep(0.2)
            else: reset += 1
            if reset >= 5: 
    ...
    def driveY(self, y, err=0.2):
        print 'doing drive y'
        self.stop()
        count = 0
        reset = 0
        while not rospy.is_shutdown() and count < 10:
            check_pos = self.srv_ok_position(String('xy'), err).ok and self.srv_ok_position(String('yaw'), err).ok
    ...
    def turnRelative(self, degree, err=0.05):
        ...
        rospy.sleep(1.5)
        while not rospy.is_shutdown() and count < 10:
            check_pos = self.srv_ok_position(String('xy'), err).ok and self.srv_ok_position(String('yaw'), err,String(user)).ok
            print 'pos: %s'%check_pos
            if check_pos:
                print 'if'
                count += 1
    ...
    def depthAbs(self, depth, err=0.2):
        self.stop()
        ...
        while not rospy.is_shutdown() and count < 10:
            if self.srv_ok_position(String('z'), err).ok and self.srv_ok_position(String('yaw'), err).ok:
                count += 1
            else: reset += 1
            if reset >= 5:
                count = 0
