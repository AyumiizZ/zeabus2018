...
#-------------------------------- for service ---------------------------------------------
		...
		self.value_xy_x = 0
		self.value_xy_y = 0
		self.value_distance_x = 0
		self.value_distance_y = 0
		self.value_depth = 0
		self.value_yaw = 0
		...
		self.waiting_time = QtGui.QLineEdit("2")
		...
		self.value_service_distance_x = QtGui.QLineEdit()
		...
		self.value_service_distance_y = QtGui.QLineEdit()
	...
	def call_xy(self):
		try:
			print("Call Service xy")
			result = self.service_xy( self.value_xy_x , self.value_xy_y)
	...
	def call_depth(self):
		try:
			print("Call Service depth")
			result = self.service_depth( self.value_depth)
	...
	def call_distance(self):
		try:
			print("Call Service distance")
			result = self.service_depth( self.value_distance_x , self.value_distance_y)
	...
	def call_yaw(self):
		try:
			print("Call Service depth")
			result = self.service_yaw( self.value_yaw)
